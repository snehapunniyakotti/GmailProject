...
//	@Value("${file.attachments.dir}")
	...
	public ResponseEntity<Object> fetchEmails(String folder, int page, int size) throws Exception {
...
//				mails = emailRepo.findBySnoozed(true, pageable); // return duplicates becoz , one email can present in many folder
...
//		mmh.setFrom(from);
	...
	public ResponseEntity<Object> saveDraftEmail(MailTemplete email, MultipartFile[] files, String messageId)
			throws Exception {
	...
	public String saveDraftEmailInGmail(MailTemplete email, MultipartFile[] files, String msgId,
		...
		try (Store store = gmailStoreService.getStore()) {
			...
			/// reopen the folder to get message id
			// get the message id for further manipulation
			draftFolder.open(Folder.READ_WRITE);
	...
	public void forwardEmail(List<Long> ids, String[] to, String[] cc, String[] bcc, String folderName,
			String initialContent, MultipartFile[] Files) throws Exception {
		...
		try (Store store = gmailStoreService.getStore()) {
			...
			for (long id : ids) {
				...
				EmailMetadata eMsg = emailRepo.findById(id)
						.orElseThrow(() -> new Exception("Email meta data not found for this id"));
...
//				helper.setText("Forwarded message:\n\n" + messageToForward.getContent(), true);
	...
	public void replyEmail(Long id, String folderName, String initialContent, MultipartFile[] Files) throws Exception {
...
//				helper.setText("Forwarded message:\n\n" + messageToForward.getContent(), true);
	 ...
	 * 
	 */
	public void starManipulation(long id, boolean isStarred, String folderName, String msgId) throws Exception {
...
//	long id, boolean isRead, String folderName, String msgId ,ReadAndUnReadDTO req
	public void readManipulation(List<Long> ids, boolean isRead, String folderName) throws Exception {
	...
	public void moveEmailToTrash(List<Long> ids, String folderName) throws Exception {
		...
		if (mdatas.isEmpty()) {
			...
			throw new Exception("Email meta data not found for this list of id");
		}
				...
				try (Store store = gmailStoreService.getStore()) {
					...
					for (EmailMetadata mdata : mdatas) {
						...
						for (EmailMetadata emd : res) {
							emd.setDeleteFlag(true); // marks as delete for faster ui re
						}
	...
	/// can not to handle for INBOX category (updates,social, forums, promotions)
	/// can not update the labels within INBOX category using IMAP
	public void UpdateLabelsToEmail(List<UpdateLabelsInEmailDTO> reqList) throws Exception {
				...
				for (UpdateLabelsInEmailDTO req : reqList) {
					...
					if (message == null) {
						log.error("UpdateLabelsToEmail : Message not found for UID: " + uid + " in folder: "
								+ searchFolder);
						throw new Exception("Message not found for UID: " + uid + " in folder: " + searchFolder);
					}

					if (!(message instanceof GmailMessage)) {
						log.error("UpdateLabelsToEmail : Message is not a GmailMessage, UID: " + uid);
						throw new Exception("Message is not a GmailMessage, UID: " + uid);
					}
	...
	/// in important folder as well
	/// need to update this method as move to folder but not removing from source
	/// folder.
	/// not moving but copying
	///
	public ResponseEntity<Object> importantFlagManipulation(List<Long> ids, String folderName) throws Exception {
	...
	public void archiveEmail(List<Long> ids, String folderName) throws Exception {
...
//				Folder archiveFolder = store.getFolder(newFolderName);
	...
	public void deleteForever(List<Long> ids, String folderName) throws Exception {
	...
	public ResponseEntity<Object> searchEmailUsingFiltersInDB(GmailSearchFilter filter) throws Exception {
			...
			for (String searchFolder : searchFolders) {
				...
				if (filter.getFromAdd() != null && !filter.getFromAdd().isEmpty()) {
					...
					if (responseEmails.isEmpty())
						...
						responseEmails.retainAll(response);// retainAll will make that only common message should
															// present in
															// responseEmails
				}
	...
	private List<DraftFileDetails> multiFileUploadInLocalDir(MultipartFile[] files) throws Exception {
		List<DraftFileDetails> fileList = new ArrayList<DraftFileDetails>();
