...
public class EmailScheduler {
	...
	public void sendScheduledEmails() {
		...
		System.err.println(" schedule Email is called !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  ");
		List<ScheduledEmail> emailsToSend = schEmailRepo.findByStatusAndScheduledTimeBefore(MailUtil.Status.PENDING,
				LocalDateTime.now());

		System.out.println(" emailsToSend  size of the shedule email !!!!!!!!!!!!!! " + emailsToSend.size());
		...
		for (ScheduledEmail email : emailsToSend) {
			try {
				...
				if (email.getGmailMessageId() != null && !email.getGmailMessageId().isEmpty()) {
					...
					System.err.println("email.getGmailMessageId()  in shedule mail  &&&&&& "+email.getGmailMessageId() + " MailUtil.GmailSchedule  &&&&&&&& "+MailUtil.GmailSchedule);
					...
					for (EmailMetadata emd : emdList) {
						...
						try (Store store = gmailStoreService.getStore()) {
							...
							if (deleteMsgForever != null) {
								...
								System.out.println("Message with UID " + uid + " not found in folder: " + MailUtil.GmailSchedule);
							}

							System.out.println("updated in IMAP !!!!!!!!!!!!!!!!!! ");  
				...
				System.err.println("Failed to send scheduled email with ID: " + email.getId());
				e.printStackTrace();
			}
		}
	}
